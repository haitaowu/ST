...
+ (BOOL) printWithPNG_JPG:(UIImage *)img offset:(int) xoff
{
    if (xoff<0) return NO;
    Byte BayerPattern[8][8] =
    {
...
+ (BOOL) drawBmp:(int) start_x startY:(int) start_y bmpPath:(NSString *) bmp_Path
{
    Byte BayerPattern[8][8] =
				{
                    {0, 32,  8, 40,  2, 34, 10, 42},
    ...
    UIImage *image = [UIImage imageNamed:bmp_Path];
    int imageWidth = image.size.width;
    int imageHeight = image.size.height;
    size_t bytesPerRow = imageWidth * 4;
    uint32_t* rgbImageBuf = (uint32_t*)malloc(bytesPerRow * imageHeight);
    ...
    NSString *pStr1 =[NSString stringWithFormat:@"CG %d %d %d %d ",bytesPerLine,imageHeight,start_x,start_y];
    //NSUInteger bytes = [pStr1 lengthOfBytesUsingEncoding:NSUTF8StringEncoding];
    
    NSData *testData = [pStr1 dataUsingEncoding: NSUTF8StringEncoding];
    NSUInteger bytes =[testData length];
    Byte *testByte = (Byte *)[testData bytes];
    
    Byte lineData[(bytesPerLine)*imageHeight+bytes+2];
    for(int i=0;i<bytes;i++)
    {
        lineData[i] = testByte[i];
    }

    for (int n = 0; n < imageHeight; n++)
    {
        for ( int m=0;m<bytesPerLine;m++) lineData[(bytesPerLine)*n+bytes+m]=bitmap[n][m];
    }
    lineData[bytesPerLine*imageHeight+bytes]=0x0d;
    lineData[bytesPerLine*imageHeight+bytes+1]=0x0a;

    NSData *data=[[NSData alloc] initWithBytes:lineData length:((bytesPerLine)*imageHeight+bytes+2)];
